...
if [ -f /data/ssh_keys ]; then
	...
	if [ ! -z "$AUTH_TOKEN" ] || [ ! -z "$RESIN_API_KEY" ]; then
		if [ -z "$RESIN_API_KEY" ]; then
			...
			curl -s -k -H "Authorization: Bearer $AUTH_TOKEN" 'https://api.resin.io/ewa/user__has__public_key?$select=id,title,public_key' | extract_json_value 'public_key' > /data/ssh_keys
		else
